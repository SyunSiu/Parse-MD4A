 ...
 */
public class SharkBarrelDistortionLinePipe extends SharkAbsLinePipe {

    private SharkProgram mProgram;

    private SharkBarrelDistortionMesh object3D;

    private SharkDirector mDirector;
    ...
    public SharkBarrelDistortionLinePipe(DisplayModeManager displayModeManager) {
        mDisplayModeManager = displayModeManager;
        mConfiguration = displayModeManager.getBarrelDistortionConfig();
        mProgram = new SharkProgram(SharkLibrary.ContentType.BITMAP);
        mDirector = new SharkDirectorFactory.OrthogonalImpl().createDirector(0);
        object3D = new SharkBarrelDistortionMesh();
    ...
    public void takeOver(int totalWidth, int totalHeight, int size) {
        ...
        glCheck("SharkBarrelDistortionLinePipe glClear");
    }
    ...
    private void draw(int index){
        ...
        mProgram.use();
        glCheck("SharkBarrelDistortionLinePipe mProgram use");
    ...
    private class SharkBarrelDistortionMesh extends SharkAbsObject3D {

        private static final String TAG = "SharkBarrelDistortionMesh";
        private int mode;
        private FloatBuffer singleTexCoordinateBuffer;

        public SharkBarrelDistortionMesh() {
        }
        ...
        private void applyBarrelDistortion(int numPoint, float[] vertexs) {
            ...
            for (int i = 0; i < numPoint; i++){
                ...
                SharkUtil.barrelDistortion(mConfiguration.getParamA(),
