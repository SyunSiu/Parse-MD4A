 ...
 */
public class SharkBitmapTexture extends SharkTexture {

    private static final String TAG = "SharkBitmapTexture";
    private SharkLibrary.IBitmapProvider mBitmapProvider;
    private Map<String,AsyncCallback> mCallbackList = new HashMap<>();
    private boolean mIsReady;
    private AsyncCallback callback;

    public SharkBitmapTexture(SharkLibrary.IBitmapProvider bitmapProvider) {
    ...
    @Override
    public boolean texture(SharkProgram program) {
    ...
    private void textureInThread(int textureId, SharkProgram program, Bitmap bitmap) {
        notNull(bitmap,"bitmap can't be null!");

        if (isEmpty(textureId)) return;

        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);
        glCheck("SharkBitmapTexture glActiveTexture");

        // Bind to the texture in OpenGL
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureId);
        glCheck("SharkBitmapTexture glBindTexture");
        ...
        glCheck("SharkBitmapTexture texImage2D");

        GLES20.glUniform1i(program.getTextureUniformHandle(),0);
        glCheck("SharkBitmapTexture textureInThread");
    }
