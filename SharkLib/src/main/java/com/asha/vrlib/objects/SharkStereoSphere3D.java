 ...
 */
public class SharkStereoSphere3D extends SharkAbsObject3D {

    private SharkDirection direction = SharkDirection.HORIZONTAL;

    public SharkStereoSphere3D(SharkDirection direction) {
    ...
    private static void generateSphere(SharkAbsObject3D object3D, SharkDirection direction) {
        generateSphere(18, 75, 150, object3D, direction);
    }

    private static void generateSphere(float radius, int rings, int sectors, SharkAbsObject3D object3D, SharkDirection direction) {
        ...
        for(r = 0; r < rings + 1; r++) {
            for(s = 0; s < sectors + 1; s++) {
                ...
                if (SharkDirection.VERTICAL == direction){
                    texcoords[t] = s*S;
                    texcoords2[t] = s*S;
                    t++;
