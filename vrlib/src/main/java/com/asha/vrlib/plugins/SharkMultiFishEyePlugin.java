 ...
 */
public class SharkMultiFishEyePlugin extends SharkAbsPlugin {

    private SharkProgram mProgram;

    private SharkProgram mBitmapProgram;

    private SharkTexture mTexture;

    private ProjectionModeManager mProjectionModeManager;

    private SharkMesh mConverterObject3D;

    private SharkDirector mFixedDirector;

    private SharkDrawingCache mDrawingCache;

    public SharkMultiFishEyePlugin(SharkMainPluginBuilder builder, float radius, SharkDirection direction) {
        mTexture = builder.getTexture();
        mProgram = new SharkProgram(builder.getContentType());
        mBitmapProgram = new SharkProgram(SharkLibrary.ContentType.BITMAP);

        mProjectionModeManager = builder.getProjectionModeManager();

        mFixedDirector = new SharkDirectorFactory.OrthogonalImpl().createDirector(0);
        mConverterObject3D = new SharkMesh(radius, direction);
        mDrawingCache = new SharkDrawingCache();
    }
    ...
    public void init(Context context) {
        ...
        SharkObject3DHelper.loadObj(context, mConverterObject3D);
    }
    ...
    private class SharkMesh extends SharkAbsObject3D {

        private static final String TAG = "SharkMesh";

        private final SharkDirection direction;

        private final float radius;

        public SharkMesh(float radius, SharkDirection direction) {
            this.radius = radius;
        ...
        private void generateMesh(SharkAbsObject3D object3D){
            ...
            for(r = 0; r < rows + 1; r++) {
                for(s = 0; s < columns + 1; s++) {
                    ...
                    if (direction == SharkDirection.HORIZONTAL){
