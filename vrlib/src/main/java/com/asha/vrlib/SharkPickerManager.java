package com.asha.vrlib;

import android.content.Context;
import android.view.MotionEvent;

import com.asha.vrlib.common.SharkMainHandler;
import com.asha.vrlib.common.SharkUtil;
import com.asha.vrlib.model.SharkRay;
import com.asha.vrlib.plugins.ISharkHotspot;
import com.asha.vrlib.plugins.SharkAbsPlugin;
import com.asha.vrlib.plugins.SharkPluginManager;
import com.asha.vrlib.strategy.display.DisplayModeManager;
import com.asha.vrlib.strategy.projection.ProjectionModeManager;

import java.util.List;


/**
 * Created by hzqiujiadi on 16/8/7.
 * hzqiujiadi ashqalcn@gmail.com
 */
public class SharkPickerManager {

    private static final String TAG = "SharkPickerManager";

    private static final int HIT_FROM_EYE = 1;

    private static final int HIT_FROM_TOUCH = 2;

    private boolean mEyePickEnable;

    private DisplayModeManager mDisplayModeManager;

    private ProjectionModeManager mProjectionModeManager;

    private SharkPluginManager mPluginManager;

    private SharkLibrary.IEyePickListener mEyePickChangedListener;

    private SharkLibrary.ITouchPickListener mTouchPickListener;

    private EyePickPoster mEyePickPoster = new EyePickPoster();

    private TouchPickPoster mTouchPickPoster = new TouchPickPoster();

    private SharkLibrary.IGestureListener mTouchPicker = new SharkLibrary.IGestureListener() {
        @Override
        public void onClick(MotionEvent e) {
            rayPickAsTouch(e);
        }
    };

    private SharkAbsPlugin mEyePicker = new SharkAbsPlugin() {
        @Override
        protected void init(Context context) {

        }

        @Override
        public void beforeRenderer(int totalWidth, int totalHeight) {

        }

        @Override
        public void renderer(int index, int width, int height, SharkDirector director) {
            if (index == 0 && isEyePickEnable()){
                rayPickAsEye(width >> 1, height >> 1, director);
            }
        }

        @Override
        public void destroy() {

        }

        @Override
        protected boolean removable() {
            return false;
        }
    };

    private SharkPickerManager(Builder params) {
        this.mDisplayModeManager = params.displayModeManager;
        this.mProjectionModeManager = params.projectionModeManager;
        this.mPluginManager = params.pluginManager;
    }

    public boolean isEyePickEnable() {
        return mEyePickEnable;
    }

    public void setEyePickEnable(boolean eyePickEnable) {
        this.mEyePickEnable = eyePickEnable;
    }

    private void rayPickAsTouch(MotionEvent e) {
        float x = e.getX();
        float y = e.getY();
        int size = mDisplayModeManager.getVisibleSize();
        if (size == 0){
            return;
        }

        int itemWidth = mProjectionModeManager.getDirectors().get(0).getViewportWidth();

        int index = (int) (x / itemWidth);
        if (index >= size){
            return;
        }
        SharkRay ray = SharkUtil.point2Ray(x - itemWidth * index, y, mProjectionModeManager.getDirectors().get(index));

        ISharkHotspot hotspot = pick(ray, HIT_FROM_TOUCH);

        if (ray != null && mTouchPickListener != null){
            mTouchPickListener.onHotspotHit(hotspot, ray);
        }
    }

    private void rayPickAsEye(float x, float y, SharkDirector director) {
        SharkRay ray = SharkUtil.point2Ray(x, y, director);
        pick(ray, HIT_FROM_EYE);
    }

    private ISharkHotspot pick(SharkRay ray, int hitType){
        if (ray == null) return null;
        return hitTest(ray, hitType);
    }

    private ISharkHotspot hitTest(SharkRay ray, int hitType) {
        List<SharkAbsPlugin> plugins = mPluginManager.getPlugins();
        ISharkHotspot hitHotspot = null;
        boolean hasHit = false;
        for (SharkAbsPlugin plugin : plugins) {
            if (plugin instanceof ISharkHotspot) {
                ISharkHotspot hotspot = (ISharkHotspot) plugin;
                hasHit = hotspot.hit(ray);
                if (hasHit){
                    hitHotspot = hotspot;
                    break;
                }
            }
        }

        switch (hitType) {
            case HIT_FROM_TOUCH:
                // only post the hotspot which is hit.
                if (hasHit){
                    mTouchPickPoster.setRay(ray);
                    mTouchPickPoster.setHit(hitHotspot);
                    SharkMainHandler.sharedHandler().post(mTouchPickPoster);
                }
                break;
            case HIT_FROM_EYE:
                mEyePickPoster.setHit(hitHotspot);
                SharkMainHandler.sharedHandler().postDelayed(mEyePickPoster, 100);
                break;
        }

        return hitHotspot;
    }

    public SharkLibrary.IGestureListener getTouchPicker() {
        return mTouchPicker;
    }

    public SharkAbsPlugin getEyePicker() {
        return mEyePicker;
    }

    public static Builder with() {
        return new Builder();
    }

    public void setEyePickChangedListener(SharkLibrary.IEyePickListener eyePickChangedListener) {
        this.mEyePickChangedListener = eyePickChangedListener;
    }

    public void setTouchPickListener(SharkLibrary.ITouchPickListener touchPickListener) {
        this.mTouchPickListener = touchPickListener;
    }

    private class EyePickPoster implements Runnable{

        private ISharkHotspot hit;

        private long timestamp;

        @Override
        public void run() {
            SharkMainHandler.sharedHandler().removeCallbacks(this);

            if (mEyePickChangedListener != null){
                mEyePickChangedListener.onHotspotHit(hit, timestamp);
            }
        }

        public void setHit(ISharkHotspot hit) {
            if (this.hit != hit){
                timestamp = System.currentTimeMillis();

                if (this.hit != null){
                    this.hit.onEyeHitOut();
                }
            }

            this.hit = hit;

            if (this.hit != null){
                this.hit.onEyeHitIn(timestamp);
            }
        }
    }

    private static class TouchPickPoster implements Runnable{

        private ISharkHotspot hit;

        private SharkRay ray;

        @Override
        public void run() {
            if (hit != null){
                hit.onTouchHit(ray);
            }
        }

        public void setRay(SharkRay ray) {
            this.ray = ray;
        }

        public void setHit(ISharkHotspot hit) {
            this.hit = hit;
        }
    }

    public static class Builder{
        private DisplayModeManager displayModeManager;
        private ProjectionModeManager projectionModeManager;
        private SharkPluginManager pluginManager;

        private Builder() {
        }

        public SharkPickerManager build(){
            return new SharkPickerManager(this);
        }

        public Builder setPluginManager(SharkPluginManager pluginManager) {
            this.pluginManager = pluginManager;
            return this;
        }

        public Builder setDisplayModeManager(DisplayModeManager displayModeManager) {
            this.displayModeManager = displayModeManager;
            return this;
        }

        public Builder setProjectionModeManager(ProjectionModeManager projectionModeManager) {
            this.projectionModeManager = projectionModeManager;
            return this;
        }
    }

    public void resetEyePick(){
        if (mEyePickPoster != null){
            mEyePickPoster.setHit(null);
        }
    }
}
